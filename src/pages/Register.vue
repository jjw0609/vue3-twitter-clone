<template>
  <div class="flex flex-col items-center space-y-4 mt-10">
    ...
    <input @click="onRegister" v-model="password" type="password" class="rounded w-96 px-4 py-3 border border-gray-300 focus:ring-2 focus:border-primary focus:outline-none" placeholder="비밀번호" />
  ...
  setup() {
    ...
    const onRegister = async () => {
      ...
      try {
        ...
        alert('회원 가입에 성공하셨습니다. 로그인 해 주세요.');
        router.push('/login');
      } catch (e) {
        switch(e.code) {
