    ...
    <div class="flex-1 flex flex-col border-r border-color">
      ...
      <div class="bg-gray-300 h-40 relative flex-none">
        ...
        <div class="border-4 border-white bg-gray-100 w-28 h-28 rounded-full absolute -bottom-14 left-2">
          <img :src="currentUser.profile_image_url" class="rounded-full hover:opacity-90 hover:opacity-100 cursor-pointer" />
        </div>
      </div>
      <!-- profile edit button -->
      ...
      <!-- user info -->
      <div class="mx-3 mt-2">
        <div class="font-extrabold text-lg">{{ currentUser.eamil }}</div>
      ...
      <div class="flex border-b border-color mt-3">
        <div class="py-3 text-primary font-bold border-b border-primary text-center w-1/4 hover:bg-blue-50 cursor-pointer hover:text-primary">
          트윗
        </div>
      ...
      <div class="overflow-y-auto">
        <Tweet v-for="tweet in 10" :key="tweet" :currentUser="currentUser" :tweet="tweet" />
      </div>
    </div>
    <!-- trend section -->
    <Trends />
